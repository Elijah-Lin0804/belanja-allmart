...
1. **看護端操作**：於手機瀏覽器打開 GitHub Pages 網址，勾選品項、填寫備註。
2. **防呆驗證**：JavaScript 檢查是否至少選取一項或填寫備註，通過後產生時間戳記。
3. **介面視覺優化**：觸發生成清單（`generateSummary`）後，底部固定按鈕自動隱藏，讓看護專注核對；若滑回上方修改品項，按鈕則動態重新顯示。 👈 (新加入這行)
4. **雲端傳輸**：點擊傳送後，前端利用 fetch API 發送 POST 請求至 Google Web App URL。
5. **資料庫寫入**：Google Apps Script 接收 JSON 資料，自動將「時間、品項、備註、狀態（預設未處理）」寫入 Google 試算表新的一列。
6. **網頁重置**：傳送成功後，前端頁面數據自動歸零、清空並滑回頂部，完成閉環。
