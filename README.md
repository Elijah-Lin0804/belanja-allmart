...
3. **雲端傳輸**：點擊傳送後，前端利用 `fetch API` 發送 POST 請求至 Google Web App URL。
4. **資料庫寫入**：Google Apps Script 接收 JSON 資料，自動將「時間、品項、備註、狀態（預設未處理）」寫入 Google 試算表新的一列。
5. **網頁重置**：傳送成功後，前端頁面數據自動歸零、清空並滑回頂部，完成閉環。
